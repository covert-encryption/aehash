"""A variant of AEhash based on ChaCha20-Poly1305 as a drop-in replacement for AES256-GCM."""
# pip install pynacl
from hashlib import sha512
from sys import argv, exit, stderr
from unicodedata import normalize

from nacl.bindings import crypto_aead_chacha20poly1305_ietf_encrypt as chapoly
...
def aehash(password: bytes, salt: bytes, mem=500, ops=10) -> bytes:
    ...
    for i in range(ops):
        # ChaCha20-Poly1305, returns size bytes ciphertext + 16 bytes GCM tag
        buf = chapoly(buf[:size], None, nonce, key)
